...
public class MapperPlugin extends PluginAdapter {
	...
	private Properties systemPs;
	private boolean isSupportSwaggerApi = false;
	...
	public void setContext(Context context) {
		...
		commentCfg.setConfigurationType(MapperCommentGenerator.class
				.getCanonicalName());
		context.setCommentGeneratorConfiguration(commentCfg);
		systemPs = System.getProperties();
	}
	...
	public void setProperties(Properties properties) {
		...
		String isSupportSwaggerApi = this.properties.getProperty("isSupportSwaggerApi");
		if ("TRUE".equalsIgnoreCase(isSupportSwaggerApi)) {
			this.isSupportSwaggerApi = isSupportSwaggerApi.equalsIgnoreCase("TRUE");
		}
	}
	...
	/**
	 * 处理实体类的包和@Table注解以及是否支持swaggerApi
	 * 
	 * @param topLevelClass
	 * @param introspectedTable
	 */
	private void processEntityClass(TopLevelClass topLevelClass,
		...
		topLevelClass.addJavaDocLine(" *");
		topLevelClass.addJavaDocLine(" * " + new FullyQualifiedJavaType(introspectedTable.getBaseRecordType()).getShortName() + ".java");
		topLevelClass.addJavaDocLine(" * ");
		topLevelClass.addJavaDocLine(" * @Author : " + systemPs.getProperty("user.name"));
		topLevelClass.addJavaDocLine(" * @DateTime : " + DateUtils.format(new Date(),DateUtils.TO_SECOND));
		topLevelClass.addJavaDocLine(" * ");
		topLevelClass.addJavaDocLine(" * Copyright (C)1984-2017  深圳万科物业发展有限公司  All rights reserved.");
		topLevelClass.addJavaDocLine(" */");
		...
		//是否支持SwaagerApi接口JavaBean
		if(this.isSupportSwaggerApi){
			topLevelClass.addAnnotation("@ApiModel(\"表名称 : " + tableName + "\")");
		}
