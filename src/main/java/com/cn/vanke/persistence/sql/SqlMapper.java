...
public class SqlMapper {
	...
	private class MSUtils {
		...
		private void newSelectMappedStatement(String msId, SqlSource sqlSource,
			...
			MappedStatement ms = new MappedStatement.Builder(configuration,
					msId, sqlSource, SqlCommandType.SELECT).resultMaps(
		...
		private void newUpdateMappedStatement(String msId, SqlSource sqlSource,
			...
			@SuppressWarnings("serial")
			MappedStatement ms = new MappedStatement.Builder(configuration,
					msId, sqlSource, sqlCommandType).resultMaps(
					new ArrayList<ResultMap>() {
						{
							add(new ResultMap.Builder(configuration,
									"defaultResultMap", int.class,
									new ArrayList<ResultMapping>(0)).build());
						}
					}).build();
			// 缓存
